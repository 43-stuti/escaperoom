<template>
<div>
  <tablesvg @itemSelected="updateCursor"></tablesvg>
  <div id='customCursor' v-if="update">
...
<script>
import tablesvg from './svg/table.vue'
export default {
  name: 'diary',
  data () {
    return {
  ...
  methods:{
    updateCursor(value) {
      console.log('LALALA',value,this.cursorUrl[value])
      if(this.cursorUrl[value]) {
        this.customCursor = this.cursorUrl[value];
      }
    },
    onMouseMove(event) {
      console.log('KYAAA')
      if(this.customCursor) {
        let obj = document.getElementById('customCursor');
        console.log('HIHHH',obj)
        if(obj) {
